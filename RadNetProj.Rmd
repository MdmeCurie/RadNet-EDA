...
### First attempt to bind the media datasets with loop failed
...
### generates a  character list of dataframes (listofDF)
### which is passed to bind_rows() as CHAR representation of and not the df object 
### dataframe names were ultimately just hard coded into the argument of `bind_rows()`.
...
At this stage the raw dataframe now has 20 variables and 504092 observations. One variable *id* was added as part of the dataframes merge to easily identify the original source (Media Type) of the data.   
...
sumfreq_LOC <- rad_data_raw %>% group_by(LOC_NUM) %>% summarize( n = n())
summary(freq_LOC)
...
#count(rad_data_raw, CITY_NAME)    # varied top cities have 5000-8000 rows
...
half_lives <- unique(element_half_life) %>% 
  mutate(HALF_LIFE_YEAR = case_when(HALF_LIFE_TIME_UNIT =="S" ~ HALF_LIFE/3153600,
                                    HALF_LIFE_TIME_UNIT =="M" ~ HALF_LIFE/525600,
                                    HALF_LIFE_TIME_UNIT =="H" ~ HALF_LIFE/8760,
                                    HALF_LIFE_TIME_UNIT =="D" ~ HALF_LIFE/365,
                                    HALF_LIFE_TIME_UNIT =="Y" ~ HALF_LIFE/1))
...
### level/order variables with limited categories where it may be useful to 
...
### For TESTING GEO CODE And check result
#n=140
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, continentalmap}
...
In order to use functions such as `state_choropleth{choroplethr}` the dataframe must contain a column with named "region" matching the naming convention found for "region" in `state.map`. The values to be mapped must be contained in the "value" column. 
```{r echo=FALSE,  message=FALSE, warning=FALSE, stateregions}
### Revamp data to include *region* as recognized by choroplethr

### Create dataframe with correct State regions - lower case full state name
############################################################################
...
state_choropleth(bystate_numanalyses, 
                 title="Number of RADNet Analyses by State") + 
                 #zoom = c("california", "arizona", "new mexico")
...
choro1<-StateChoropleth$new(bystate_NaN)
choro1$title <- "Empty Result RADNet Entries by State"
choro1$ggplot_scale <- scale_fill_manual(name="Empty Result Entries",
...
```{r, echo=FALSE, minireviews}
### REVIEW OF DATA with smaller Subsets: HI/AK, micronesia
##########################################################
count(rad_data_raw, STATE_ABBR == "HI" & 
        !is.na(RESULT_AMOUNT))
...
ggmap(HImap) + 
  theme_map() +
  geom_count(data = subset(HI_data, !is.na(RESULT_AMOUNT)), 
             aes(x= lon, y= lat, color = MAT_ID), na.rm = TRUE,
             position = position_jitter(width = 0.1, height = 0.1)) +
  theme(legend.title = element_blank())


### Observations with no RESULT_AMOUNT
############################################
table(HI_data$ANALYTE_ID, is.na(HI_data$RESULT_AMOUNT))
#HI_data_filt <- HI_data[complete.cases(HI_data[,11]),] #filt out cases where col 11 is NA
count(HI_data, !is.na(HI_data$RESULT_AMOUNT)) 
ggplot(subset(HI_data, is.na(RESULT_AMOUNT)), aes(RESULT_DATE, fill = MAT_ID)) +
  geom_histogram(bins = 50, alpha = 0.6)

table(droplevels(HI_data$ANALYTE_NAME))


ggplot(subset(HI_data, !is.na(RESULT_AMOUNT)), aes(RESULT_DATE, fill = MAT_ID)) +
  geom_histogram(bins = 50, alpha = 0.6)

ggplot(subset(HI_data, !is.na(RESULT_AMOUNT)), aes(RESULT_DATE, fill = ANALYTE_NAME)) + 
  geom_histogram(bins = 50, alpha = 0.6)

ggplot(subset(HI_data, ( !is.na(RESULT_AMOUNT) &
                           RESULT_UNIT == 'PCI/L' & 
                           MAT_ID == 'PASTEURIZED MILK')),
              aes(RESULT_DATE, RESULT_AMOUNT, colour = ANALYTE_ID)) +
  geom_point() +
  labs(title = "Radioctive Analysis of Pasteurized Milk in Hawaii",  
       x = "Year", y = "pCi/L")

ggplot(subset(HI_data, ( !is.na(RESULT_AMOUNT) &
                           RESULT_UNIT == 'PCI/L' & 
                           (MAT_ID == 'PASTEURIZED MILK' | MAT_ID == "DRINKING WATER"))),
              aes(RESULT_DATE, RESULT_AMOUNT, colour = MAT_ID)) +
  geom_point() +
  labs(title = "Radioctive Analysis of Pasteurized Milk and Drinking Water in Hawaii",  
       x = "Year", y = "pCi/L")

ggplot(subset(HI_data, ( !is.na(RESULT_AMOUNT) &
                           RESULT_UNIT == 'PCI/M3' & 
                           (MAT_ID == 'AIR-FILTER' | MAT_ID == "AIR-CHARCOAL"))),
              aes(RESULT_DATE, RESULT_AMOUNT, colour = CITY_NAME)) +
  geom_point() +
  labs(title = "Radioctive Analysis of Air Samples in Hawaii",  
       x = "Year", y = "pCi/m3")
...
ggplot(subset(HI_data, ( !is.na(RESULT_AMOUNT) &
              ...
              aes(RESULT_DATE, RESULT_AMOUNT, colour = ANALYTE_ID)) +
  geom_point() +
  labs(title = "Radioctive Analysis of Precipitation Samples in Hawaii",  
       x = "Year", y = "pCi/L")
...
ggplot(subset(HI_data, ( !is.na(RESULT_AMOUNT) &
              ...
              aes(RESULT_DATE, RESULT_AMOUNT, colour = CITY_NAME)) +
  geom_point() +
  labs(title = "Potassium-40 Samples in Hawaii",  
       x = "Year", y = "pCi/L")


table(subset(HI_data, RESULT_UNIT == "ACI/L"))
#length(unique(HI_data$SAMP_ID))
#str(HI_data_$SAMP_ID)
```
...
################################################
ggplot(rad_data_raw, aes(RESULT_DATE, fill = MAT_ID)) + 
  geom_histogram(bins = 50)
ggplot(rad_data_raw, aes(LOC_NUM, fill = MAT_ID)) + 
  geom_histogram(bins = 50)
ggplot(rad_data_raw, aes(RESULT_AMOUNT, fill = MAT_ID)) + 
  geom_histogram(bins = 50) +
  scale_y_log10() +
  scale_x_log10()
...
###################################################
### PLOTS Preliminary Overview with  HI DATA  #####
###################################################
ggplot(HI_data, aes(ANALYTE_ID)) + geom_bar()

### (1) Plot of Result Amounts from all sources, units 
min(HI_data[,"RESULT_AMOUNT"])
...
ggplot(HI_data, aes(x=RESULT_DATE, y= RESULT_AMOUNT, 
                         color = MAT_ID)) +
  geom_point() +
  facet_wrap(~HI_data$RESULT_UNIT, scales = "free") +
...
### (3a) Plot of HI Beta Results (pCi/m3)
#########################################
ggplot(subset(HI_data, RESULT_UNIT == "PCI/M3" & ANALYTE_ID =="BETA"),
...
### (3b) Plot of Beta Results elsewhere (pCi/m3)
#####################################
ggplot(subset(rad_data_raw, !is.na(RESULT_UNIT) &
              RESULT_UNIT == "PCI/M3" &
              ANALYTE_ID =="BETA"),
...
choro_fukushima1$title <- 
  "Max Gross Beta (pCi/m3) for Air Samples"
...
ggplot(subset(rad_data_raw, !is.na(RESULT_UNIT) &
  ...
  #             x= as.Date("1984-03-04"), #moved for display point at 1981-03-04
  #             # y= 1.25) 
  labs(title = 'pCi/L in RADNet Samples 1978-2017', 
       caption = 'EPA RadNet Data') +
  scale_x_date(date_breaks = "5 years", labels = date_format("%Y")) +
  xlab('Date') +
  ylab('pCi/L') +
...
#Doesnt want to facet with subset() 
#ggplot(subset(HI_data, !is.na(RESULT_AMOUNT)),
#        aes(RESULT_AMOUNT)) + 
#   geom_histogram() +
#   scale_y_log10() +
