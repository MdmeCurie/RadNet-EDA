...
A quick inspection of the dataframe and the contents of each of the variables is undertaken in order to understand the breadth of the data and potential problems.
Overall the raw dataframe now has 20 varibles and 504092 observations.  One variable *id* was added as part of the merge to easily identify the original source (Media Type) of the data.   
...
#### (@)Geocoding Locations
Using the package `ggmap` it is possible to obtain the latitude and longitude of locations listed in the dataframe from the city and state information. With this information it will be possible to represent data and information geographically.
...
Rather than inundate the API with multiple requests for the same location a separate dataframe is created with only the unique places. 
...
geo_result <- geocode(places[9,3], output = c("latlona"))
places[9,4] <- as.numeric(geo_result[2])
places[9,5] <- as.numeric(geo_result[1])
places[9,6] <- as.character(unlist(geo_result[3]))
places[9,6]
places[9,6] == places[9,3]

### Split and strip geocode CITY STATE for comparison to citystate
banana <- strsplit(gsub('[^[:alnum:] ]',"", places[9,6]), " +")[[1]]
test_geo <-paste(toupper(banana[1]), toupper(banana[2]))
miss_geo <- character()
if (test_geo != places[9,3]) {miss_geo <- c(miss_geo, places[9,6])}
...
for (i in 1:nrow(places)) {
  ...
  Sys.sleep(1)  # pause for API request
}
...
USAMap
continental <- places %>% filter(lon < -75 & lon > -120 & 
                                      lat < 50 & lat > 20)
...
hawaii <- places %>% filter(lon < -154 & lon > -161 & 
                                      lat < 23 & lat > 17)
