...
An interesting observation is that 16% (81,921) observations have no result entry, result amount is NA. This seems odd as RadNet is designed specifically for the documentation of analtye concentrations, i.e., results.  A separate dataframe was created which filters out the empty RESULT_AMOUNT observations, *rad_data*.
...
A quick corrlation matrix heat map indicates that most numerical variables are not correlated. 

A character correlation matrix^[^ggpairs] on a 10% sample of the database helps clarify where the majority of the observations may be classified. 
...
#ggcorr(rad_data_raw[,c(4,6,8,12:14,18,22,23,28,29)])
ggcorr(rad_data_raw[,numeric_cols])
...
A strong correlation (r=0.87) emerges when plotting the observations vs number of monitoring stations within a region. This correlation while still strong is weaker (r=0.79) when we examine the number of empty results to regional monitoring stations. Using a choropleth map we can plot the number of observations entered by state and compare information by location. It is easy to pick out individual states with moderate/low number of observations and higher percentages of empty results, e.g., Kentucky with <4,500 entries has > 17% empty, while California with many more samples (>19,000) has <15% NA results. 
...
The material ID appears to be the most convenient way to quickly subset and classify the observations. As seen in the univariate analysis one material (Air-Filter) dominates and matches to  other variables help to fill out the story. The correlation matrix confirms that Air-Filter samples coincide with top categorical results: result unit *pCi/m^3^*, analytical procedure *1*, type *R*adioactive, and comprises the most complete entries, *Y*.

As to the incomplete entries, the seem to occupy some niche positions, the Air-Charcoal entries are predominantly incomplete. The milk and precipitation samples appear to have more incomplete entries percentage-wise than the remaining material types. There are no incomplete *E*lement observations. 

The *E*lement category is itself an interesting, but small subset of the data. Likely due to its small representation element observations are not generally split in any other categories, they are all completed drinking water samples reported in g/L analysed mostly by procedure 9 with some analysed by procedure 57. On this limited set it was easy to tie procedures 9 and 57 to the analytes potassium (K) and calcium (Ca), respectively. 
...
A somewhat surprising result is the weak correlation of the analytical procedure number to the result amounts.  While the result is dependent on the method used for the analysis, the assignment of a procedure number to that method could be completely random. Therefore, it would not necessarily be expected that any particular procedure number should correlate to a particular result value, give that the results themselves can be random. There are a few implications, the procedure are grouped in way to assemble like results and that the results themselves possess a low variance when analysed by a particular method.

### What was the strongest relationship you found?
The strongest correlations which exist in the original dataframe is between the result amounts, minimal detectable concentration and combined standard uncertainty values which is to be anticipated as both are determined/calculated from analyses of samples. A strong negative (-1) correlation between the Y/N complete variables confirms that these variables were set up correctly.



Again some issues with recording data hampers ana
...
#####Beta Results 
Taking the results for the beta analyes a simple face by location helps get a bigger picture of the spike events. Within the Hawaiian data set the locations are limited to Hawaiian cities. If the entire dataframe is presented in the same manner we can elucidate that the July 2008 appears localized to Tennessee, while the March 1981 incident was picked up at several monitoring stations.

Research did not yield any information on any nuclear incidents during 2008.  While the earlier 1981 spike in NV is entered as March 3, the only nuclear accident found in this timeline was at the Tusraga Power Plant[^tsuruga], reported as being on March 8th[^tsuruga8] or 9th.[^tsuruga9], however, the reports were covered up for some 40 days and earlier issues at the power plant went unreported. 
