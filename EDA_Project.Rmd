...
```{result_units, echo=FALSE, message=FALSE, warning=FALSE}
### RESULT_UNIT ->  
...
#####Result Dates 
A simple distribution of entries over result dates, yields typical daily count ranges of 125 to 250 entries, but regular spikes of 50% to 100% above the baseline are noted. These spikes occur on annual intervals; until 2009 the spikes are at mid-year (7/1) afterwhich they appear year-end (12/31). Grouping the observations by a 2 year date range gives a bar plot in which you can see a drop in entries around 1989 and again 10 years later, though there is a subtle increase from 1999 to 2012. The drop in 2017 cannot be verified as the data was only downloaded through September.
...
print("HALF-LIFE OF GROSS BETA Entries")
summary(rad_data_raw %>% select(ANALYTE_ID, HALF_LIFE) %>% filter(ANALYTE_ID == "BETA"))
```

Fify-six elements have half-lives, but the top 10 account for a majority of the remaining dataframe.
...
#### Dataset structure:
The original dataframe had 504,092 entrieswith 19 variables. The variables represent factors (some levelled), numerical values, characters and dates. Some of the variables were recast for uniformity and to help data manipulation and representation. Most notably the dates were converted to date characters in ISO 8601 format.
...
The dates were grouped into 3 different time spans to provide a way for a more simplified examination of the dataframe as well as to investigate duplicate Sample IDs where entries for different analytes are made on different days. 
...
A couple other features of note:  1) There is a long tailed positive skew for the distribution of sample replicates. While there is the possibility for multi-analyte analysis most samples are primarily sampled for gross beta analysis. 1) The periodic spike in entries occurs July 1 and December 31. Is this a side effect of the database, precipitated by some administrative speciifcation or were there really that many year-end entries, on New Year's Eve?  
...
Using choropleth maps we can visualize the number of observations entered by state and compare information by location. It is easy to pick out individual states with moderate/low number of observations and higher percentages of empty results, e.g., Kentucky with <4,500 entries has 18%% empty, while California with 23,686 entries has 13% NA results.  

```{r uschoropleth, echo=FALSE,  message=FALSE, warning=FALSE}
#### Choropleth of Sample Analyses by State Setup ####
...
#### Choropleths of Entries by State ####
...
However, a simple summary of the percent empty observations shows the median percent empty entries for region is 15.9%, as there are only a few outliers so in general empty entries are not a location issue. 
...
Further examination of the Hawaiian air samples confirms that around April 2011, 378 air analyses were entered. One-half as air-charcoal (190) with 128 empty and the remainder as air-filter analyses (188) with only 21 incomplete. However, upon a closer look, the air-filter sample spike is actualy earlier than the air-charcoal spike. So while interesting, more information is needed to establish the actual between the empty air-charcoal and air-filter samples. A quick survey of the databases shows that only a few locations have empty air-charcoal entries with Alaska and Hawaii having the greatest number. 
...
```{r sample_status, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r second_date, echo=FALSE, message=FALSE, warning=FALSE}
#### Reduce dataframe size for KNITR ####
...
```{r radium_csu_mdc, echo=FALSE, message=FALSE, warning=FALSE}
all_radium +
  geom_line(aes(x=RESULT_DATE, y= MDC, colour = "MDC"), lty = 3 )+
  geom_line(aes(x=RESULT_DATE, y= CSU, colour = "CSU"), lty = 5 )+
...
The mpty entries do have a strong correlation to material ID. Air-Charcoal entries are predominantly incomplete. Milk and precipitation samples have more incomplete entries percentage-wise than the remaining material types. The largest group of sample material, Air-Filter, has the least incomplete entries.

####Interesting relationships between minor features
The *E*lement category is a small subset of the data and is not generally split in any other categories. Most succinctly, element analysis consists of *completed drinking water samples* reported in *g/L*. The element entries are split by analytical procedure with most being analysed by procedure 9 with some analysed by procedure 57. On this limited set it was easy to tie the procedures to the analytes potassium (K) and calcium (Ca), respectively. 
...
With the distribution of samples over the various monitoring stations positively skewed, just like the skew of the monitoring station location ID.  However, if sample counts are displayed over region they look more evenly distributed because the regions are not organized in the same fashion. 
...
```{r byyear_facet, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(subset(rad_trim, ANALYTE_ID %in%
  ...
  geom_bar() +
  scale_y_sqrt() +
  labs(title = "", x = "LOCATION NUMBER", y = "COUNT")
```

####Result Amounts and Locations - Beta Analyses 
Using only beta analyes we facet by location to get a bigger picture view of the release events noted earlier. The Hawaiian data set shows that the Chernobyl event was picked up at the Honolulu station(s), which appears to be the only station monotoring at that time, but the Fukushima release was picked up by both Kahuku and Kauai spikes in gross beta.

```{r beta_regions, echo=FALSE, message=FALSE, warning=FALSE}
#### HI Beta Results (pCi/m3) by Date, Coloured for location #######
HI_beta <- ggplot(subset(HI_data, !is.na(RESULT_AMOUNT) &
                           RESULT_UNIT == "PCI/M3" & 
                           ANALYTE_ID =="BETA"),
...
```{r allbeta_regions, echo=FALSE, message=FALSE, warning=FALSE }
...
```{r fukushima_choro, echo=FALSE, message=FALSE, warning=FALSE}
...
Not other radionuclides were analysed, but plot of the other analytes with a consistent (pCi/m^3^) during this event shows some areas with peaks up to 37 pCi/m^3^. 
...
Measurements in Alaska after Fukushima were almost twice the US-contiguous response, 1.15 pCi/m^3^  vs. 0.6 pCi/m^3^ gross beta max. Other radionuclides were detected at even higher values in AK, 144 pCi/m^3^ for Bismuth-212, 126 pCi/m^3^ for Lead-212 and 59 pCi/m^3^ for Thallium-208.
...
```{r entry_status, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r sample_complete, echo=FALSE, message=FALSE, warning=FALSE }
...
To show the status of samples the proportion of completeness (incompleteness) is faceted by material. The pComplete is complimentary to the pIncomplete and consequently the boxplots are a goup of inversed mirror images. However, we can easily see that air-filter, drinking water and surface water samples are largely complete with some sample outliers. The dispersion is greater for the other material categories. Both air-charcoal and pastuerized milk completed samples are left skewed for completeness with the median almost at the third-quartile. The distribution for precipitation is the most normal with the median centered between the 2nd and 3rd quartiles. 
...
summary(subset(samples_rank, MAT_ID = 'PRECIPITATION' & percent == "pC"))
```
...
##reload USA Map re-centered to include Alaska
usa_center = c(-115,39)
USAMap = ggmap(get_googlemap(center=usa_center, scale=2, zoom=3, 
                             extent ="device"))
...
Entries with results seems to have some interesting story and more difficult to track than firs thought. These entries at first glance are a fairly significant portion of the database (16%). However when grouped as samples (sample ID and date) reveals that most empty observations are just multiple analyte tests on the same sample. So for the most part each sample has some information. With the exception of the air-charcoal samples in Hawaii, status and location are not correlated. 
...
ggplot() +
  ...
  labs(title="Complete/Empty Periodic Entries", x=NULL, y = "Count",
       subtitle ="Result Entries Made July 1 and December 31") +
